...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#TO-DO:
#Re-write away from on switches, use if/else.
...
pep_resource_open_pre(*OUT){
  on($KVPairs.resc_hier like "Archive"){
    *svr="server.one.surfsara.nl";
    *dma=dmattr($KVPairs.physical_path, *svr, $KVPairs.logical_path);                            #DMF meta attribute update
    # *dma="(NEW)               100%"; #Debug values, could comment out the previous line and use this.
    *dmfs=substr(*dma, 1, 4);
    *stg=triml(*dma, "        ");
...
iarch(){
  *svr="server.one.surfsara.nl";     #Resource Server FQDN
